...
docker run --rm \
  ...
  -e VERSION="$VERSION" \
  "$CONTAINER_NAME" \
  /bin/bash -c "nvflare provision -p \$PROJECT_YML && ./_injectLiveSyncIntoStartupKits.sh \$PROJECT_YML && ./_generateStartupKitArchives.sh \$PROJECT_YML \$VERSION" || { echo \"Docker run failed\"; exit 1; }
