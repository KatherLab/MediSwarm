...
NOTES = [
    ...
    # 15 AL concept (extended after the talk, 17 Sep)
    "Active learning. The problem this deliverable solves is simple: the expensive part of our work is not the training, it is the labelling. Every labelled case costs a radiologist's time. "
    "So if a radiologist gives us time for twenty more cases, which twenty should we ask for? The naive answer is any twenty. Active learning says: ask the model. "
    "Follow the diagram from left to right. On the left is a pool of cases with images but no label. The model gives each case three numbers: the probability of no lesion, of benign, of malignant. "
    "For an easy case those look like 95, 3 and 2 percent. For a hard case they look like 40, 30 and 30. The second one is the case the model is unsure about, and that is what the middle box measures. "
    "Entropy just means how spread out the three numbers are; high entropy, the model is unsure. We sort all cases by that number and take the top k. "
    "That k is the labelling budget, the number of cases the radiologist has time for. The radiologist labels them, they go into the training set, we retrain, and the loop starts again. The model chooses its own homework. "
    "The dashed path at the bottom is the control: same budget, same radiologist, but the k cases are drawn at random. Any method has to beat that. "
    "One more thing, which is why the next slide counts malignant cases: on the challenge set the model calls 21 cases malignant while 37 truly are. It under-calls cancer, so malignant cases are the ones it needs most.",
    # 16 AL result (extended after the talk, 17 Sep)
    "This table is the selection step alone, before any retraining. We took the 165 challenge cases, hid their labels and treated them as the unlabelled pool; 37 of them are malignant, about 22 percent. "
    "The budget is simply how many cases you are allowed to have labelled. Both methods pick that many, we uncover the labels of the picked cases and count the malignant ones. "
    "Random column: draw the budget at random, count, repeat many times, report the average and the spread. Since 22 percent of the pool is malignant, ten random cases contain about two malignant ones; that column is just budget times 22 percent. "
    "Entropy column: the model scores all 165 cases once, sorts them by how unsure it is, and hands over the top of the list. The ten most uncertain cases contained 5 malignant ones; there is no plus or minus because there is no randomness. "
    "Budget of 20: random gets 4 or 5, the model gets 9. At 30 and 40 the ratio is still 1.6. So at every small budget, which is the realistic budget, the model finds roughly twice as many of the cases it needs. "
    "The last row is a sanity check: with 100 of 165 cases picked, both methods have taken most of the 37 malignant cases, so they meet at 21 and 22. The advantage lives where budgets are small, and that is where we are. "
    "What this does not show yet is the second half: that a model retrained on the selected cases gets better than one retrained on the random ones. That is the reduced retraining run in October and November, two arms at two budgets, about 45 hours of GPU across the fleet. When that is in, D3.2 is complete.",
    # 17 DP concept (extended after the talk, 17 Sep)
    "Differential privacy is a formal way of saying: what leaves a hospital must not reveal too much about what is inside it. Not a policy, a mathematical guarantee with a number attached. "
    "The proposal asks for two things: optional noise in training, and a privacy budget that the software enforces. "
    "The diagram follows one site through one round. The site trains and produces a model update, a long list of numbers saying how the shared model should change. Two things happen before it leaves. "
    "First, clipping: we cap the size of the update, so no single site can push the shared model further than a fixed amount. Second, noise: we add random numbers calibrated to that cap, "
    "large enough that nobody can work backwards from the update to the data, small enough that, averaged over eight sites and twenty rounds, the model still learns. Only that clipped, noised update travels. The images never leave. "
    "The box underneath is the accountant. Every noisy round spends a bit of privacy, and the accountant adds it up as a number called epsilon; smaller epsilon, stronger privacy. When the agreed budget is spent, training stops. That is the budget by design. "
    "The guarantee is at the hospital level: someone who sees the shared model cannot tell whether a given hospital took part in a round. Clipping and noise are in the software since 1.8. "
    "Still missing: the accountant writing epsilon into the metrics, the automatic stop, and the measurement of what the noise costs in accuracy.",
    # 18 DP numbers (extended after the talk, 17 Sep)
    "This table answers one question: how much noise do we have to add to get a given guarantee? The left column is the guarantee, epsilon, for a full twenty-round run. The middle column, sigma, is the amount of noise relative to the clipping cap. "
    "Bigger sigma, more noise, stronger privacy, and a worse model. Epsilon of 10 is what most published medical work uses; it needs a sigma of 2.4. Epsilon of 3 is a strong guarantee; it needs 6.7, almost three times as much noise. "
    "Epsilon of 1 is the strictest number people cite; it needs 18, and at our scale of eight sites that will most likely destroy the model. The small delta in the caption is a standard technicality, an allowed failure probability of one in a hundred thousand. "
    "Two honest limits. The guarantee covers whether a hospital took part. It does not cover whether a particular patient was in a hospital's data; that needs noise inside the local training itself, and we do not claim it. "
    "And we have not yet measured what sigma 2.4 costs in accuracy. One twenty-round run with the noise on gives us that number, and then this table becomes a real trade-off. "
    "In one line: a hospital-level guarantee at the commonly accepted level is within reach at sigma 2.4, but its cost in accuracy is unmeasured, so the consortium should choose epsilon only after that run.",
    # 19 robustness concept (extended after the talk, 17 Sep)
    "Adversarial robustness. The proposal asks for a report on how a swarm can be attacked and what protects it. The uncomfortable starting point: in a swarm every site sends model weights to whoever aggregates the round, and nobody inspects them. "
    "So every site is a potential attacker, by intent, by a bug, or through a compromised machine that sends garbage. "
    "The diagram shows one round. The eight sites send their updates, weighted by how much data they hold: UKA 52 percent, UMCU 18, down to VHIO with 0.6. One update is poisoned, and I chose the smallest site on purpose, to show that size does not protect us. "
    "The rule we use today is a plain weighted mean with no limit on how large any single update may be. If VHIO sends an update ten times too large, the mean simply takes it: the shared model moves 5.5 times VHIO's actual share. "
    "The defence at the bottom is the norm-bounded mean. Before averaging, each site's update is capped at a length proportional to that site's weight; a too-large update is shrunk to the cap, a normal one passes unchanged. "
    "Every site stays in, the data-set weights stay in. Same attack: the model moves by 0.04, and with no attack at all by 0.032. The attack has almost stopped mattering.",
    # 20 robustness numbers (extended after the talk, 17 Sep)
    "We tried five aggregation rules on a simulation with the consortium's real data-set weights. Two columns matter: the error with no attacker, which is what the rule costs us in a normal round, measured as the distance from the correct weighted average; and what happens under the attack. "
    "The weighted mean, our current rule, costs nothing but offers no protection. The norm-bounded mean costs 0.032, almost nothing, and holds the attack at 0.04. Those are the two rows to compare. "
    "The three rules below are the textbook answers. Trimmed mean throws away the largest and smallest values, coordinate median takes the middle value, Krum keeps only the one update that looks most like the others. "
    "They all bound an attacker, but look at the first column: 0.47, 0.60, 0.64. A huge error with nobody attacking, because these rules treat every site as equal and throw information away. "
    "Our sites are anything but equal: UKA holds 84 percent of all benign cases. A rule that ignores the weights aims at the average site instead of the pooled data, and that alone costs more than the attack it prevents. "
    "So the recommendation is the norm-bounded mean: cap every contribution, keep every site, keep the weights. One limit to state openly: the cap is proportional to weight, so it does not limit UKA. "
    "Protecting against the largest site too would mean capping its share, which costs accuracy; that is a governance decision for the consortium. Still open for the report: a dishonest aggregating site, which we simulate in October.",
    # 21 white-hat plan
