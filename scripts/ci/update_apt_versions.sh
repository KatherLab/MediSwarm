...
git commit docker_config/Dockerfile_ODELIA -m "WIP: remove apt versions for rebuild" || echo "[INFO] Nothing to commit (version removal unchanged). Continuing..."
...
if git diff --quiet; then
  ...
  git reset --hard HEAD || echo "[INFO] No commit to reset. Skip."
else
