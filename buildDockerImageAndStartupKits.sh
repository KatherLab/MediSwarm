...
if [ -z "$PROJECT_FILE" ]; then
    echo "Usage: buildDockerImageAndStartupKits.sh -p <swarm_project.yml> [-d <dockerfile_suffix>] [--use-docker-cache]"
    exit 1
fi

VERSION=$(./getVersionNumber.sh)
DOCKER_IMAGE="jefftud/${DOCKERFILE_SUFFIX}:$VERSION"
DOCKERFILE_PATH="docker_config/Dockerfile_${DOCKERFILE_SUFFIX}"
...
if [[ "$DOCKERFILE_SUFFIX" == "ODELIA" ]]; then
    echo "Preparing DINOv2 model weights for ODELIA build..."
    ...
    if echo 2e405cee1bad14912278296d4f42e993 $MODEL_WEIGHTS_FILE | md5sum --check - \
     && echo 153d2db1c329326a2d9f881317ea942e $MODEL_LICENSE_FILE | md5sum --check -; then
        cp -r ./docker_config/torch_home_cache "$CLEAN_SOURCE_DIR/torch_home_cache"
        chmod a+rX "$CLEAN_SOURCE_DIR/torch_home_cache" -R
    else
        echo "Model file checksum verification failed"
        exit 1
    fi
else
    echo "Skipping pre-trained model download; not required for Dockerfile_${DOCKERFILE_SUFFIX}"
fi


# build the Docker image

docker build $DOCKER_BUILD_ARGS -t "$DOCKER_IMAGE" "$CLEAN_SOURCE_DIR" -f "$DOCKERFILE_PATH"
